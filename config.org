...
#+BEGIN_SRC emacs-lisp
  (unbind-key "M-s")
  (bind-keys :prefix-map consult-search-map
             :prefix "M-s"
             :prefix-docstring "Consult search keymap prefix"
             ("v" . emacs-version))
  (use-package consult
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; A few more useful onfigurations...
  (use-package emacs
    :init
    ;; TAB ycle if there are only few candidates
    (setq completion-cycle-threshold 3)

    ;; Emas 28: Hide commands in M-x which do not apply to the current mode.
    ;; orfu commands are hidden, since they are not supposed to be used via M-x.
    ;; (setq read-extended-ommand-predicate
    ;;       #'command-completion-default-include-p)

    ;; Enable indentation+ompletion using the TAB key.
...
#+BEGIN_SRC emacs-lisp
  (use-package company
    ...
    ;:hook
    ;((lisp-mode scheme-mode java-mode c-mode c++-mode) . company-mode))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package lispy
    ...
    :hook
    ((lisp-mode scheme-mode emacs-lisp-mode) . lispy-mode))
  ...
  (use-package ac-geiser
   ...
   ;; :hook
   ;; ((geiser-mode geiser-repl-mode) . auto-complete-mode)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;  :defer t
  ;  :ensure t)
  ;(add-hook 'tuareg-mode-hook #'caml-mode)

  (use-package merlin
    :defer t
    :ensure t)
    ;; :config
    ;; (define-key merlin-mode-map  (kdb "C-g") #'merlin-complete)
