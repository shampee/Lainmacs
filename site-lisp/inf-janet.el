...
(defvar inf-janet-minor-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "\C-c\C-c" 'janet-eval-defun)
...
(defcustom inf-janet-program "janet -s -n"
...
(defun janet-re-seq (regexp string)
  "Return list of all regexp matches in a string"
  (if (= (length string) 0)
      '()
    (save-match-data
      (let ((pos 0)
...
(defcustom janet-source-modes '(janet-ts-mode)
